 ...
 *    parentDiv     容器，从哪里添加下拉复选框
 *    hiddenButtonCls  点击确定隐藏容器的按钮的样式
...
function CheckboxSelect(opt){
    ...
    this.active = null;
    this.nameHidden= opt.nameHidden || null;
    this.visible = false;
    this.init();
}
...
CheckboxSelect.prototype = {
    init: function(){
        ...
        this.hiddenButton = this.$C('button');
        //设置div的样式
        this.attr(this.container, 'class', this.containerCls);
        this.attr(this.hiddenButton, 'class', this.hiddenButtonCls);
        //将div附在DOM文档中
        /*this.doc.body.appendChild(this.container);*/
        this.parentDiv.appendChild(this.container);
        //设置div属性
        this.setPos();
        var _this = this, input = this.input,container = this.container,hiddenButton=this.hiddenButton;
        /*var textnode=document.createTextNode("保存");*/
        /* hiddenButton.appendChild(textnode);*/
        ...
        this.on(container,"mouseover",function(){
            $("body").unbind("click");
        })
        this.on(container,"mouseout",function(){
        ...
        /*this.on(hiddenButton,'click',function(e){
         e.preventDefault();
         _this.hide();
         });*/
    },
    ...
    setPos: function(){
        ...
        container.style.cssText =
            'position:absolute;overflow:hidden;background:white;z-index:2;width'
    ...
    onClick:function(e){
        ...
        if(this.data.length>0){
            ...
            for(var i=0,len=this.data.length;i<len;i++){
                ...
                $("input[name="+this.nameHidden+"]").each(function(){
                ...
                tempinput.value = this.data[i].value;
                ...
                if(checkboxdata.length>0){
                    for(var t=0;t<checkboxdata.length;t++){
                        if(checkboxdata[t] === this.data[i].value)
                            tempinput.checked = true;
                    }
                }
                item.appendChild(tempinput);
                item.setAttribute("id",this.data[i].id);
                item.appendChild(this.doc.createTextNode(this.data[i].value))
                this.items[i] = item;
                this.container.appendChild(item);
            }
            $('.checkboxselect-container').css("visibility","hidden");
            this.show();
