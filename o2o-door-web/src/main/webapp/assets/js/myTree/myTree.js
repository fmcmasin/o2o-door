$(document).ready(function(e) {
    ...
    TModuleNodeName = $('#TreeList .' + PrentNodeClass + ' .this_title');//顶层节点名称
    TModuleNode.removeClass('this_show').addClass('this_hidden');
    TModuleNode.find('.this_title').removeClass('entypo-minus-squared').addClass('entypo-plus-squared');
    if(TModuleNode.next().hasClass(ChildrenListClass)){
        TModuleNode.next().css('display','none');//紧跟的下一个是子节点
    }
    var num = 1;
    $('#addToMask').click(function(){
        AddNode(0,$(this).parent(),'模板'+num);
        num+=1;
    });

    //========================编辑区域的HTML源码==============================
    function EditHTML(NewName){
        var str = '<div class="this_title entypo-minus-squared">' + NewName + '</div>';
        str    += '<div class="editBT"></div>';
        str    += '<div class="editArea"><span>[编辑]</span><span>[添加同级目录]</span><span>[添加下级目录]</span><span>[删除]</span></div>';
        str    += '<div class="edit_next">this level</div>';
        str    += '<div class="edit_next">order</div>';
        str    += '<div class="edit_next">time</div>';
        str    += '<div class="edit_next">your name</div>';
        /* <div class="edit_next">1994.01.25</div>
         <div class="edit_next">张三</div>
         <div class="edit_next">1</div>
         <div class="edit_next">2</div>*/
        return str;
    }
    ...
    function TModuleNodeName_Click(Obj){
        if(Obj.has('input').length==0){//非编辑模式下进行
            ...
            if(tempNode.hasClass('this_hidden')){
                tempNode.removeClass('this_hidden').addClass('this_show');
                tempNode.find('.this_title').removeClass('entypo-plus-squared').addClass('entypo-minus-squared');
//                    TModuleNode.find('.this_title').removeClass('entypo-minus-squared').addClass('entypo-plus-squared');
                ...
                tempNode.removeClass('this_show').addClass('this_hidden');
                tempNode.find('.this_title').removeClass('entypo-minus-squared').addClass('entypo-plus-squared');
                if(tempNode.next().hasClass(ChildrenListClass)){
    ...
    function EditArea_Event(Obj){
        ...
        var objTitle = objParent.find('.this_title');//节点名称
        //-----------------编辑区的鼠标效果------------------
    ...
    function CheckEdititon(pNode,text){
        ...
        if(SameLevelTag!=''){
            if(text!=''){
                //---------------- 根据节点样式遍历同级节点 --------------------
                var IsExsit = false;
                pNode.parent().parent().children('div').children('.this_title').each(function(){
    ...
    function NewNodeRename(tag,pNode){
        ...
        if(tag==0){//添加同级目录
            if(pNode.attr('id')==TreeName){
                TObj = pNode.children('div').children('.this_title');
            }
            else{
                TObj = pNode.parent().parent().children('div').children('.this_title');
            }
        }
        else{//添加下级目录
            if(pNode.parent().next().html()!=null){//原来已有子节点
                TObj = pNode.parent().next().children('div').children('.this_title');
            }
            else{//没有子节点
                TObj = null;
            }
    ...
    function EditNode(obj,text){
        ...
        obj.removeClass("entypo-minus-squared");
        obj.parent().find('.editBT').html("<div class='entypo-check ok' title=确定></div><div class='entypo-cancel cancel' title=取消></div>");
        obj.has('input').children().first().focusEnd();//聚焦到编辑框内

        obj.parent().find('.ok').click(function(){
            obj.addClass("entypo-minus-squared");
            Edit_OK(obj);
        });
    ...
    function AddNode(tag,obj,NameStr){
        if(tag==0 || tag==1){
            ...
            if(tag==0){//添加同级目录
                ...
                /*console.log(obj.find(".this_title").attr("class"));*/
                obj.find(".this_title").removeClass('entypo-plus-squared').addClass("entypo-minus-squared");
                ...
                obj.attr('class',PrentNodeClass + ' this_show');//激活父节点展开状态模式
            ...
            with(newNode){
                ...
                find('.this_title').click(function(){
                    TModuleNodeName_Click($(this));
                });
            ...
            EditNode(newNode.find('.this_title'),newNode.find('.this_title').html());//添加后自动切换到编辑状态
        }
    }
